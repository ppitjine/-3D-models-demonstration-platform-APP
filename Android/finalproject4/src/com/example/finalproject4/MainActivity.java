 ...
 * @author Lee
 */
public class MainActivity  extends RendererActivity implements View.OnClickListener
{
	...
	{
    	Uri uri = Uri.parse("http://10.0.2.2/models/Brook/brook.obj");
		Intent it = new Intent(Intent.ACTION_VIEW, uri); 
		it.setType("text/*");
		scene.lights().add(new Light());
		//add from
		IParser parser = Parser.createParser(Parser.Type.OBJ,
				getResources(), "com.example.finalproject4:raw/camaro_obj", true);
		parser.parse();
		
		_cube = parser.getParsedObject();
		_cube.scale().x = _cube.scale().y = _cube.scale().z = .7f;
		//add here
